...
module.exports = function (io) {
    ...
    io.on('connection', function (socket) {
        ...
        interval2 =  setInterval(()=>{
           ...
           UserInfo.DeviceListSocket({
            userid: data.userId, page: data.currentPage, limit: data.limit,
            areanumber: data.areanumber, devicestatus: data.devicestatus
        }, result => {
               let results=({
        ...
        socket.on("ondatainfo", (data) => {
            ...
            interval = setInterval(() => {
                ...
                UserInfo.deviceAreaInfoDetails({ deviceid: data.deviceid, userid: data.userid }, result => {
                   // socket.emit("deviceDataInfo", result);
                   io.in(data.userid).emit("deviceDataInfo", result)
                  // io.sockets.emit("deviceDataInfo", result)
                });
    ...
    io.on('connection', function (socket) {
        socket.on('onrealtimedata1', function(data1){  
            // if (interval1) {
            //     clearInterval(interval1);onrealtimedata1
            // }

            interval1 = setInterval(() => {
                socket.join("notify"+data1.userId);
                UserInfo.DeviceListSocketNotification({
                    userid : data1.userId
                }, result => {
                    let results = {
                        deviceList : result,
